...
class Listify_Customizer {
	...
	public static function init() {
		...
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'customizer_scripts_preview' ), 99 );
	 ...
	 * @return void
	 */
	public static function customizer_scripts_preview() {
		if ( ! is_customize_preview() ) {
			return;
		}

		wp_enqueue_script( 'listify-customizer-preview', get_template_directory_uri() . '/inc/customizer/assets/js/customizer-preview.js', array( 'jquery' ), '', true );

		$colors = Listify_Customizer_Utils::get_regex_theme_mods( 'color-' );
		$type = Listify_Customizer_Utils::get_regex_theme_mods( 'typography-' );

		wp_localize_script( 'listify-customizer-preview', 'listifyCustomizerPreview', array(
			'styleControls' => array_keys( array_merge( $colors, $type ) ),
			'typeControls' => array_keys( $type )
		) );
	}
