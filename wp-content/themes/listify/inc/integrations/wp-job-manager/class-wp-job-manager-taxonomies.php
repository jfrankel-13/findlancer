...
class Listify_WP_Job_Manager_Taxonomies {
	...
	public function add_term_fields() {
		...
		<div class="form-field term-thumbnail-wrap">
			<label><?php _e( 'Thumbnail', 'listify' ); ?></label>
			...
			<script type="text/javascript">
				...
				jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
					...
					file_frame = wp.media.frames.downloadable_file = wp.media({
						title: '<?php _e( "Choose an image", "listify" ); ?>',
						button: {
							text: '<?php _e( "Use image", "listify" ); ?>'
						},
						multiple: false
					});
	...
	public function edit_term_fields( $term ) {
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label><?php _e( 'Thumbnail', 'listify' ); ?></label></th>
			<td>
	...
	public function term_columns( $columns ) {
		...
		$new_columns['thumb'] = __( 'Image', 'listify' );
