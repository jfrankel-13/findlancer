...
class Listify_WP_Job_Manager extends listify_Integration {
	...
	public function init() {
		...
		$this->tags = new Listify_WP_Job_Manager_Tags;
	...
	public function job_manager_output_jobs_defaults( $default ) {
		...
		if ( is_tax( 'job_listing_type' ) ) {
			...
			$default[ 'show_tags' ] = true;
		} elseif ( is_tax( 'job_listing_category' ) ) {
			$default[ 'show_categories' ] = true;
			$default[ 'categories' ] = $type->slug;
			$default[ 'selected_category' ] = $type->slug;
		} elseif ( is_tax( 'job_listing_tag' ) ) {
			$default[ 'show_tags' ] = true;
			$default[ 'tags' ] = $type->slug;
			$default[ 'selected_tag' ] = $type->slug;
		} elseif ( is_search() ) {
		...
		if ( isset( $_GET[ 'search_tags' ] ) ) {
            $tags = array_filter( array_map( 'esc_attr', $_GET[ 'search_tags' ] ), 'listify_array_filter_deep' );

            if ( ! empty( $tags ) ) {
                $default[ 'selected_tag' ] = $tags[0];
            }
